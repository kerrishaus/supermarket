...
export class LoadingState extends State
{
    init(stateMachine)
    {
        ...
        function registerEventListeners()
        {
            ...
            {
                pointerMoveOrigin.x = ( event.touches[0].clientX / window.innerWidth ) * 2 - 1;
                pointerMoveOrigin.y = - ( event.touches[0].clientY / window.innerHeight ) * 2 + 1;
            ...
            {
                pointerMoveOrigin.x = ( event.clientX / window.innerWidth ) * 2 - 1;
                pointerMoveOrigin.y = - ( event.clientY / window.innerHeight ) * 2 + 1;
            ...
            window.addEventListener("keydown", (event) =>
            {
                ...
                else
                {
                    switch (event.code)
                    {
                        ...
                        case "ArrowRight":
                            break;
                            move = MoveType.Keyboard;
                            moveTarget.quaternion.copy(player.quaternion);
                            break;
                    };
                }
            });
        ...
        let move = null;
        let keys = new Array();
        let pointerMoveOrigin = new THREE.Vector2();
        let moving = false, pointerMove = false, freeCam = false;
